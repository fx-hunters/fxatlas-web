...
**Divurve** — 외화 목표·환전 타이밍 제안 서비스. 이 레포(프론트엔드)는 **표시와 입력 UI를 전담**합니다. 계산(몬테카를로, 버킷 분리, 변동성 등)은 백엔드 API에서 받은 결과를 그대로 표시합니다.
...
각 원칙은 **왜(요약) → 하지 말 것(❌) → 할 것(✅) → 강제 수단** 순서로 읽는다. 예시는 Divurve 도메인(goals·plans·safeRatio·currencyCode 등)에 맞춘다. "권장"으로 표기한 ESLint 규칙은 아직 `.eslintrc.cjs`에 없으므로 도입 시 활성화한다(§11 참고).
